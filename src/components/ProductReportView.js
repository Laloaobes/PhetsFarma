...
import { BarChart2, Loader2, Download, FileSpreadsheet } from 'lucide-react';
...
// Función de formato para la nueva estructura de datos en CSV
const formatListForCSV = (list = []) => {
    // \n crea una nueva línea dentro de una celda de CSV para una mejor legibilidad
    return list.map(item => `${item.name} (${item.qty.toLocaleString('es-MX')} pz)`).join('\n');
};
...
const ProductReportPDF = ({ reportData, dateRange, formatCurrency, user, totals }) => {
    const styles = StyleSheet.create({
        ...
        participantText: { fontSize: 7, padding: 0 },
        participantItem: { marginBottom: 2 } // Espacio entre elementos de la lista en PDF
    });
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.table}>
                    <View style={[styles.tableRow, styles.tableHeader]}>
                        ...
                        <Text style={styles.colSellers}>Vendedores (Pz)</Text>
                        <Text style={styles.colDistributors}>Distribuidores (Pz)</Text>
                    </View>
                    {reportData.map(item => (
                        <View style={styles.tableRow} key={item.productName} wrap={false}>
                            ...
                            <View style={[styles.colSellers, styles.participantText]}>
                                {(item.sellers || []).map(seller => (
                                    <Text key={seller.name} style={styles.participantItem}>{`${seller.name} (${seller.qty.toLocaleString('es-MX')} pz)`}</Text>
                                ))}
                            </View>
                            <View style={[styles.colDistributors, styles.participantText]}>
                                {(item.distributors || []).map(distributor => (
                                    <Text key={distributor.name} style={styles.participantItem}>{`${distributor.name} (${distributor.qty.toLocaleString('es-MX')} pz)`}</Text>
                                ))}
                            </View>
                        </View>
                    ))}
...
export default function ProductReportView({
    ...
    const handleCsvDownload = () => {
        ...
        const headers = ['Producto', 'Piezas Vendidas', 'Monto Total', 'Vendedores (Pz)', 'Distribuidores (Pz)'];
        const rows = reportData.map(item => [
            escapeCsvCell(item.productName),
            item.totalQty,
            item.totalAmount,
            escapeCsvCell(formatListForCSV(item.sellers)),
            escapeCsvCell(formatListForCSV(item.distributors))
        ]);
                        ...
                        <table className="min-w-full bg-white text-sm">
                            <thead className="bg-slate-100">
                                <tr className="border-b-2 border-slate-200">
                                    ...
                                    <th className="p-3 font-semibold text-slate-600 text-left">Vendedores (Pz)</th>
                                    <th className="p-3 font-semibold text-slate-600 text-left">Distribuidores (Pz)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {(reportData || []).map(item => (
                                    <tr key={item.productName} className="border-b border-slate-200 hover:bg-blue-50">
                                        <td className="p-3 font-medium text-slate-900 align-top">{item.productName}</td>
                                        <td className="p-3 text-right text-slate-700 align-top">{(item.totalQty || 0).toLocaleString('es-MX')} pz</td>
                                        <td className="p-3 text-right font-semibold text-slate-900 align-top">{formatCurrency(item.totalAmount)}</td>
                                        <td className="p-3 text-slate-700 text-xs align-top">
                                            <ul className="list-none p-0 m-0 space-y-1">
                                                {(item.sellers || []).map(seller => (
                                                    <li key={seller.name}>{`${seller.name} (${seller.qty.toLocaleString('es-MX')} pz)`}</li>
                                                ))}
                                            </ul>
                                        </td>
                                        <td className="p-3 text-slate-700 text-xs align-top">
                                            <ul className="list-none p-0 m-0 space-y-1">
                                                {(item.distributors || []).map(distributor => (
                                                    <li key={distributor.name}>{`${distributor.name} (${distributor.qty.toLocaleString('es-MX')} pz)`}</li>
                                                ))}
                                            </ul>
                                        </td>
                                    </tr>
