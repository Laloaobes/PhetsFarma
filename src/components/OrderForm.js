...
import { ShoppingCart, User as UserIcon, Truck, DollarSign, Plus, Trash2, FlaskConical, Search, ChevronDown, Check, X } from 'lucide-react';
...
// Nuevo componente para la selección de productos con búsqueda
const ProductSearchSelect = ({ products, value, onSelect, disabled }) => {
  ...
  return (
    <div className="relative">
      <div className="flex items-center w-full border rounded-md bg-white text-sm" onClick={() => setIsOpen(!isOpen)}>
        <input
          type="text"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          placeholder="Buscar producto..."
          className="flex-grow p-2 focus:outline-none bg-white rounded-l-md"
          disabled={disabled}
        />
      </div>
      {isOpen && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
        ...
        <div className="col-span-1 md:col-span-2">
          ...
          <div className="flex items-center">
            ...
            <span className="font-medium text-gray-800 p-2 bg-white border rounded-md w-full text-sm">{formatNumber(item.price)}</span>
          </div>
        </div>
        {/* Descuento por ítem */}
        <div className="col-span-1 md:col-span-1">
...
export default function OrderForm({ 
  ...
  // Estados para añadir cliente, representante y distribuidor
  ...
  const handleSubmit = () => {
    if (!client || !laboratory || items.some((i) => !i.productName || (parseFloat(i.price) <= 0 && i.productName === ""))) {
      setModalMessage("Por favor, completa el laboratorio y el cliente. Asegúrate de que todos los productos seleccionados tengan nombre.");
      setShowModal(true);
      return;
    }
              ...
              {isAddingNewRep ? (
                ...
                <>
                  <select id="representative-select" value={representative} onChange={(e) => setRepresentative(e.target.value)} className="w-full bg-transparent focus:outline-none" disabled={user && (user.role === 'Representante' || user.role === 'Gerente de laboratorio')}>
                    ...
                    {[...representatives].sort((a, b) => a.name.localeCompare(b.name)).map(r => <option key={r.id} value={r.name}>{r.name}</option>)}
                  </select>
              ...
              {isAddingNewDist ? (
                ...
                <>
                  <select id="distributor-select" value={distributor} onChange={(e) => setDistributor(e.target.value)} className="w-full bg-transparent focus:outline-none" disabled={user?.role === 'Representante'}>
                    ...
                    {[...distributors].sort((a, b) => a.name.localeCompare(b.name)).map(d => <option key={d.id} value={d.name}>{d.name}</option>)}
                  </select>
          ...
          <div className="text-right w-full max-w-xs">
            ...
            {itemLevelDiscountAmount > 0 && (
              <div className="flex justify-between py-1 border-t">
                ...
                <span className="font-medium text-red-500">-${formatNumber(itemLevelDiscountAmount)}</span>
              </div>
            )}
            <div className="flex justify-between py-2 border-t mt-2">
              <span className="text-xl font-bold text-gray-800">{itemLevelDiscountAmount > 0 ? 'Total Final:' : 'Total del Pedido:'}</span>
              <span className="text-xl font-bold text-gray-900">${formatNumber(finalTotal)}</span>
            </div>
          </div>
        </div>
